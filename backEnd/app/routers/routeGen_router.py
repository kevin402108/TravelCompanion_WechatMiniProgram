...
routeGen_logger = setupLogger( "route_generate" , fileName= "route_generate.log" )
securitySchema = HTTPBearer()
...
async def routeAutoGenerate(
    ...
    credentials:HTTPAuthorizationCredentials = Depends(securitySchema),
    ...
    try:
        ...
        try:
            with db.begin():
                ...
                routeGen_logger.info( f"开始往route表插入新生成的旅游路线记录 插入记录的详细信息:{new_route}" )
                db.add(new_route)
                db.flush()

                routeGen_logger.info( f"route表插入新生成的旅游路线记录成功，新生成的旅游路线id:{new_route.id}" )
                insertRouteSpotMapping(db, spot_ids, new_route.id)

                if db.query(RouteSpotsMapping).filter_by(route_id=new_route.id).count() == 0:
                    routeGen_logger.error( f"route_id={new_route.id} 未插入任何关联数据" )
                    raise HTTPException(500, detail="数据插入失败!")
        except Exception as e:
            db.rollback()
            raise HTTPException(status_code=500, detail=f"SQL执行出错：{str(e)}")
...
def insertRouteSpotMapping(db: Session, spotIDList: list[str], route_id: int):
    ...
    converted_SpotIDList = safeSpotlistConvert( spotIDList )
        ...
        try:
            with db.begin_nested():
                ...
                if missingIDs := set(converted_SpotIDList) - existingIDs:
                    ...
                    routeGen_logger.error( error_msg )
                ...
                if not newSpots:
                    routeGen_logger.warning( f"route_id={route_id} 所有景点均已关联，无需插入" )
                    return
            ...
            if e.orig.args[0] == 3024:
                routeGen_logger.error( f"QUERY TIMEOUT route_id:{route_id}" )
            ...
            routeGen_logger.error( f"TOTAL OPERATION TIMEOUT (route_id:{route_id})" )
            raise HTTPException(
...
def safeSpotlistConvert( spot_ids: list[str ] ) -> list[int ]:
    if not spot_ids:
